...
> **현재 상태: Vercel 배포 완료.** 전 페이지(회사소개·로보틱스/AI/엔지니어링·뉴스·문의)·SEO·301 리다이렉트·문의폼 개인정보 동의/처리방침 구현 완료. 남은 핵심 작업은 커스텀 도메인 연결과 배포 후 문의폼 실전송 확인.
...
| 3 | 문의 폼 | **✅ 구현 (Web3Forms)** | 홈 #contact 섹션에 폼 통합, 개인정보 동의 체크 후 메일 전송 |
| 4 | 법적/실무 | **✅ 문의폼 개인정보 대응 추가** | 개인정보 수집·이용 동의 체크박스 및 처리방침 페이지 추가 |
...
- [x] 문의 — 홈 `#contact` 섹션에 폼 통합(Web3Forms). 별도 `/contact` 페이지 대신 통합, 개인정보 동의 체크 후 전송
...
- **문의 폼 (#3)**: ✅ Web3Forms로 구현 완료(홈 #contact). `NEXT_PUBLIC_WEB3FORMS_KEY` 설정 후 개인정보 수집·이용 동의 체크박스를 통과해야 전송됩니다.
- **법적 (#4)**: ✅ 원본에 없던 개인정보 처리방침 페이지(`/[lang]/privacy`)와 문의폼 동의 체크박스를 추가했습니다. 실제 문구는 회사 정책/법무 검토 대상입니다.
...
1. **문의폼 실전송 확인** — Vercel 환경 변수 반영 후 재배포, 실제 문의 제출 및 수신 메일 확인
2. **커스텀 도메인 연결** — Vercel Settings → Domains에서 `core-robotics.kr` 연결. 공식 도메인 사용 시 `NEXT_PUBLIC_SITE_URL=https://www.core-robotics.kr`로 변경
